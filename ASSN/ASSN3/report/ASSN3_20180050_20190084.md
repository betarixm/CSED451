...
# 개요
기존의 ASSN2에서 구현했던 탄막 슈팅게임을 3D로 확장하여 재구현 하는 것이 목표이다.
게임 내부 기능 (backend)는 거의 동일하고 object 들을 2D->3D로 확장하고, 와이어프레임 렌더링과 3D 카메라 제어가 주요 목표이다.
**따라서 본 보고서에는 ASSN2에서 변경 및 확장된 부분에 대해서만 자세히 다룰 예정이며 기존의 게임 구현과 동일한 부분 (class, method, 실행방법 등)은 작성하지 않았다.**
